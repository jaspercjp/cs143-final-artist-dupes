...
class VGG19(nn.Module):
    ...
    def forward(self, x, input_type='content'):
        ...
        for i,layer in enumerate(self.features):
            x = layer(x)
            if i in self.content_layers: # Choose layer(s) to get the content representation from
                F.append((i, x))
            if not (self.style_layers is None) and (i in self.style_layers): # Choose layers to get style representations from
                G.append((i, gram(x)))
                
        if not self.style_layers is None:
            return F, G
        else:
            return F
...
if __name__ == "__main__":
    ...
    style_im, style_im_shape = load_image_as_tensor("sample-van-gogh.jpg", l=256)
    ...
    output_img.save("style-recon.jpg")
